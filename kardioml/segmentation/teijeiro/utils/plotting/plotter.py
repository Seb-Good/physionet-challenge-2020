...
import kardioml.segmentation.teijeiro.utils.pyperclip as pyperclip
...
def constraints_network_graphviz(interpretation, outfile):
    ...
    """
    lst = []
    pats = sorted(interpretation.patterns, key=lambda p: -ap.get_obs_level(p.automata.Hypothesis))
    for pat in pats:
    ...
    for node in G.nodes_iter():
        for obs in observations:
            if node == str(id(obs.start)) or node == str(id(obs.end)):
                ...
                node.attr['label'] = str(obs.earlystart) if node == str(id(obs.start)) else str(obs.lateend)
                break
...
def parallel_plot(signals, fig=None):
    ...
    for i in range(1, len(signals) + 1):
        ...
        fig.add_subplot(len(signals), 1, i, sharex=shared_axes).plot(signals[i - 1])
    return fig
...
class ObservationVisualizer(object):
    ...
    def draw(self):
        ...
        for i in range(LEVELS):
            plot.hlines(min_sig + i * self.lev_height, 0, len(self.signal), color='#DCDCDC')
        ...
        for obs in observations:
            ...
            plot.fill(
                [pair[0] for pair in self.trapezs[obs]],
            ...
            bottom = self.sig_limits[0] + level * self.lev_height
            plot.plot([obs.time.start, obs.time.end], [bottom, bottom + self.lev_height], color=color)
        # Focus draw
        if self.interpretation.focus:
            ...
            ell = Ellipse(
                xy=[xmin + (xmax - xmin) / 2, y],
                width=max(
                    m2s(250), min(len(self.signal) - focus.earlystart, focus.lateend - focus.earlystart)
                ),
        ...
        self.fig.gca().set_xbound(lower=last_point - 1000, upper=last_point + 1000)
        self.fig.gca().set_ybound(
            lower=min_sig - (self.sig_limits[1] - min_sig) / 10,
            upper=self.sig_limits[1] + (self.sig_limits[1] - min_sig) / 10,
        )
        self.fig.show()
...
def plot_branch(interpretation, fig=None, label_funcs=None, target=None, full_tree=False):
    """
    ...
    """
    view = InterpretationVisualizer(interpretation, fig, label_funcs, target, full_tree)
    view.draw()
    return view
...
class InterpretationVisualizer(object):
    ...
    def __init__(self, interp, fig=None, label_funcs=None, target=None, full_tree=False):
        """
                ...
                if dist < 10:
                    ...
                    if event.button == 1:
                        if node in self._subfigs:
                            mgr = Gcf.get_fig_manager(self._subfigs[node].fig.number)
                            mgr.window.activateWindow()
                            mgr.window.raise_()
                        else:
                            # Else plot the observations
                            signal = sig_buf.get_signal(sig_buf.get_available_leads()[0])
