...
from kardioml.segmentation.teijeiro.utils.MIT.MITAnnotation import read_annotations, MITAnnotation
from kardioml.segmentation.teijeiro.utils.MIT import get_leads
...
def _merge_annots(annotators):
    ...
    while True:
        ...
        if result and nxt.time - result[-1].time <= C.RDEFLECTION_MIN_DIST:
            result[-1] = max(nxt, result[-1], key=lambda ann: (ann.num, -ann.time))
        else:
            result.append(nxt)
...
def _load_annots():
    ...
    annotators = []
    for lead in leads:
        command = ['gqrs', '-r', rec, '-outputName', annotator, '-m', '0.1', '-s', lead]
    ...
    for ann in _merge_annots(annotators):
        ...
        if ann.time - refann[idx - 1].time > C.RDEFLECTION_MIN_DIST and (
            idx >= len(refann) or refann[idx].time - ann.time > C.RDEFLECTION_MIN_DIST
        ):
