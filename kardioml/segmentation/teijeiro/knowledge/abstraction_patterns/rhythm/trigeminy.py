...
def _get_measures(pattern, ectopic=False):
    ...
    # RR
    rrs = np.diff([beats[i].time.start for i in range(len(beats)) if _is_ectopic(i) == ectopic])
    if not ectopic:
    ...
    for twave in pattern.evidence[o.TWave]:
        if twave is not pattern.finding:
            qrs = next(q for q in reversed(beats) if q.lateend <= twave.earlystart)
...
def _reg_ae_tconst(pattern, qrs):
    ...
    const = Iv(
        min(2 * refrr - C.RR_MAX_DIFF, refrr * C.COMPAUSE_MIN_F),
    ...
    mincompause = max(C.COMPAUSE_MIN_DUR, min(minrr * C.COMPAUSE_RREXT_MIN_F, minrr + C.COMPAUSE_RREXT_MIN))
    tnet.add_constraint(beats[idx - 1].time, qrs.time, Iv(mincompause, maxrr * C.COMPAUSE_RREXT_MAX_F))
...
def _reg_nae_tconst(pattern, qrs):
    ...
    tnet.add_constraint(prev.time, qrs.time, Iv(rrev - C.RR_MAX_DIFF, rrev + C.RR_MAX_DIFF))
    BASIC_TCONST(pattern, qrs)
    tnet.add_constraint(qrs.start, qrs.end, C.NQRS_DUR)
    tnet.set_before(qrs.time, hyp.end)
    # Constraints with the precedent T Wave
...
def _ect_qrs_tconst(pattern, qrs):
    ...
    if idx > 0:
        ...
        if pattern.evidence[o.Cardiac_Rhythm] and idx == 1:
            ...
            tnet.add_constraint(
                prev.time, qrs.time, Iv(C.TACHY_RR.start, max(C.TACHY_RR.start, prevrr - C.TMARGIN))
            )
...
def _cycle_finished_gconst(pattern, _):
    ...
    pattern.hypothesis.meas = o.CycleMeasurements((np.mean(rrs), np.std(rrs)), (rtm, rtst), (pqm, pqst))


TRIGEMINY_PATTERN = PatternAutomata()
TRIGEMINY_PATTERN.name = "Trigeminy"
TRIGEMINY_PATTERN.Hypothesis = o.Trigeminy
TRIGEMINY_PATTERN.add_transition(0, 1, o.Cardiac_Rhythm, ENV, _prev_rhythm_tconst)
# N
TRIGEMINY_PATTERN.add_transition(1, 2, o.QRS, ENV, _env_qrs_tconst)
# V
TRIGEMINY_PATTERN.add_transition(2, 3, o.QRS, ABS, _ect_qrs_tconst, _ect0_gconst)
# N
TRIGEMINY_PATTERN.add_transition(3, 4, o.QRS, ABS, _reg_ae_tconst, _cycle_finished_gconst)
# N
...
# N
TRIGEMINY_PATTERN.add_transition(6, 7, o.QRS, ABS, _reg_ae_tconst, _cycle_finished_gconst)
...
# N
TRIGEMINY_PATTERN.add_transition(19, 20, o.QRS, ABS, _reg_ae_tconst, _cycle_finished_gconst)
