...
def _reg_qrs_tconst(pattern, qrs):
    ...
    if pattern.get_evidence_type(qrs)[1] is ENV:
        ...
        if idx == 2:
            ...
            mrr, srr = hyp.meas.rr
            const = Iv(
                min(ref2rr - 2 * C.RR_MAX_DIFF, 2 * mrr - 4 * srr),
        ...
        refcompause = (
            ...
            if idx > 2
            else maxrr * C.COMPAUSE_RREXT_MAX_F
        )
        mincompause = max(
            C.COMPAUSE_MIN_DUR,
            maxrr,
            min(minrr * C.COMPAUSE_RREXT_MIN_F, refcompause - C.TMARGIN, minrr + C.COMPAUSE_RREXT_MIN),
        )
        tnet.add_constraint(beats[idx - 1].time, qrs.time, Iv(mincompause, maxrr * C.COMPAUSE_RREXT_MAX_F))
        # Beats cannot overlap
        tnet.add_constraint(beats[idx - 1].end, qrs.start, Iv(C.TQ_INTERVAL_MIN, np.Inf))
...
def _get_measures(pattern, even=0):
    ...
    # RR
    rrs = (
        np.diff([beats[i].time.start for i in range(len(beats)) if i % 2 == even and len(beats) - i <= n])
        / 2.0
    )
    # RT
    rts = []
...
BIGEMINY_PATTERN.add_transition(0, 1, o.Cardiac_Rhythm, ENV, _prev_rhythm_tconst, _prev_rhythm_gconst)
# Necessary evidence are QRS complexes.
# N
BIGEMINY_PATTERN.add_transition(1, 2, o.QRS, ENV, _reg_qrs_tconst)
# V
BIGEMINY_PATTERN.add_transition(2, 3, o.QRS, ABS, _ect_qrs_tconst, _ect0_gconst)
# N
BIGEMINY_PATTERN.add_transition(3, 4, o.QRS, ABS, _reg_qrs_tconst, _cycle_finished_gconst)
# V
BIGEMINY_PATTERN.add_transition(4, 5, o.QRS, ABS, _ect_qrs_tconst)
# N
BIGEMINY_PATTERN.add_transition(5, 6, o.QRS, ABS, _reg_qrs_tconst, _cycle_finished_gconst)
...
# N
BIGEMINY_PATTERN.add_transition(15, 16, o.QRS, ABS, _reg_qrs_tconst, _cycle_finished_gconst)
