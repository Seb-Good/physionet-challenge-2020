...
def _prev_rhythm_gconst(pattern, rhythm):
    ...
    verify(not isinstance(rhythm, o.Asystole) and type(pattern.hypothesis) != type(rhythm))
...
def _qrs2_tconst(pattern, qrs):
    ...
    elif pattern.evidence[o.Cardiac_Rhythm] and isinstance(
        pattern.evidence[o.Cardiac_Rhythm][0], o.Asystole
    ):
...
def _qrs3_tconst(pattern, qrs):
    ...
    if beats.index(qrs) == 1:
        tnet.add_constraint(beats[0].time, qrs.time, Iv(C.TACHY_RR.start + C.RR_MAX_DIFF, C.BRADY_RR.end))
    # If we have reached an initial state.
    if pattern.istate == 0:
        idx = beats.index(qrs)
        meanrr, stdrr = pattern.hypothesis.meas.rr
        minrr = beats[1].time.start - beats[0].time.end if idx == 2 else meanrr - stdrr
        tnet.add_constraint(
            beats[idx - 1].time,
            qrs.time,
            Iv(min(C.ASYSTOLE_RR.start, minrr + C.RR_MAX_DIFF), C.ASYSTOLE_RR.start),
        )
        # The block time has to be higher than the mean RR plus the standard
        # deviation.
        if meanrr > 0:
            tnet.add_constraint(
                beats[idx - 1].time, qrs.time, Iv(meanrr + stdrr, max(meanrr + stdrr, C.ASYSTOLE_RR.start))
            )
...
def _qrsn_tconst(pattern, qrs):
    ...
    meanrr, stdrr = pattern.hypothesis.meas.rr
    rr_bounds = Iv(min(C.ASYSTOLE_RR.start, meanrr - stdrr + C.RR_MAX_DIFF), C.ASYSTOLE_RR.start)
...
def _qrs_gconst(pattern, _):
    ...
    if len(beats) == 3:
        ...
        if beats[1].time.start - beats[0].time.start < min(0.9 * refrr, refrr - stdrr):
            verify(beats[2].time.start - beats[0].time.start > refrr * C.COMPAUSE_MAX_F)
            verify(beats[2].time.start - beats[1].time.start > refrr + C.COMPAUSE_MIN_DUR)
...
RHYTHMBLOCK_PATTERN.add_transition(
    0, 1, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst, _prev_rhythm_gconst
)
RHYTHMBLOCK_PATTERN.add_transition(
    0, 2, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst, _prev_asyst_gconst
)
...
RHYTHMBLOCK_PATTERN.add_transition(3, 4, o.QRS, ABSTRACTED, _qrs3_tconst, _qrs_gconst)
...
RHYTHMBLOCK_PATTERN.add_transition(6, 4, o.QRS, ABSTRACTED, _qrsn_tconst, _qrs_gconst)
