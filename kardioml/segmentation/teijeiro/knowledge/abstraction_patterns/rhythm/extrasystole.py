...
def _prev_rhythm_tconst(pattern, rhythm):
    ...
    tnet.add_constraint(
        pattern.hypothesis.start, pattern.hypothesis.end, Iv(2 * C.TACHY_RR.start, 3 * C.BRADY_RR.end)
    )
...
def _prev_rhythm_gconst(pattern, rhythm):
    ...
    verify(type(pattern.hypothesis) is o.Asystole or type(pattern.hypothesis) != type(rhythm))
...
def _qrs_rref_tconst(pattern, qrs):
    ...
    if pattern.evidence[o.Cardiac_Rhythm]:
        pattern.last_tnet.set_before(qrs.end, pattern.evidence[o.Cardiac_Rhythm][0].end)
...
def _qrs_env_tconst(pattern, qrs):
    ...
    if pattern.evidence[o.QRS].index(qrs) == 1:
        ...
        tnet.add_constraint(prev.time, qrs.time, Iv(C.TACHY_RR.start, C.BRADY_RR.end))
...
def _qrs_ext_tconst(ventricular):
    ...
    def tconst(pattern, qrs):
        ...
        if idx > 0:
            tnet.add_constraint(beats[idx - 1].time, qrs.time, Iv(C.TACHY_RR.start, 0.9 * C.BRADY_RR.end))
        ...
        if pattern.istate == 0:
            ...
            if refrr is not None:
                ...
                tnet.add_constraint(
                    beats[idx - 1].time, qrs.time, Iv(C.TACHY_RR.start, max(C.TACHY_RR.start, refrr - short))
                )
...
def _qrs_fin_pause_tconst(pattern, qrs):
    ...
    if pattern.istate == 0:
        ...
        # Reference RR
        minrr = beats[1].time.start - beats[0].time.end if len(beats) == 4 else pattern.hypothesis.meas.rr[0]
        maxrr = beats[1].time.end - beats[0].time.start if len(beats) == 4 else pattern.hypothesis.meas.rr[0]
        # Compensatory pause
        tnet.add_constraint(
            beats[-3].time,
            qrs.time,
            Iv(min(minrr + C.COMPAUSE_MIN_DUR, minrr * C.COMPAUSE_MIN_F), maxrr * C.COMPAUSE_MAX_F),
        )
        # Advanced beat RR
        ...
        tnet.add_constraint(
            beats[idx - 1].time,
            qrs.time,
            Iv(C.TACHY_RR.start * C.COMPAUSE_MIN_F, C.BRADY_RR.start * C.COMPAUSE_RREXT_MAX_F),
        )
...
def _qrs_fin_npause_tconst(pattern, qrs):
    ...
    if pattern.istate == 0:
        ...
        minrr = beats[1].time.start - beats[0].time.end if len(beats) == 4 else pattern.hypothesis.meas.rr[0]
        maxrr = beats[1].time.end - beats[0].time.start if len(beats) == 4 else pattern.hypothesis.meas.rr[0]
        tnet.add_constraint(beats[-3].time, qrs.time, Iv(minrr - C.RR_MAX_DIFF, maxrr + C.RR_MAX_DIFF))
        # The last QRS should have the same morphology than the one before the
        # extrasystole.
        qrs.shape = beats[-3].shape
...
def _extrasyst_gconst(pattern, _):
    ...
    if pattern.istate == 0:
        ...
        # same shape.
        verify((beats[-3].paced and beats[-1].paced) or signal_match(beats[-3].shape, beats[-1].shape))
...
EXTRASYSTOLE_PATTERN.add_transition(
    0, 1, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst, _prev_rhythm_gconst
)
...
EXTRASYSTOLE_PATTERN.add_transition(3, 4, o.QRS, ABSTRACTED, _qrs_ext_tconst(False), _qrs_ext_gconst)
...
EXTRASYSTOLE_PATTERN.add_transition(4, 7, o.QRS, ABSTRACTED, _qrs_fin_pause_tconst)
EXTRASYSTOLE_PATTERN.add_transition(6, 7, o.QRS, ABSTRACTED, _qrs_fin_pause_tconst)
EXTRASYSTOLE_PATTERN.add_transition(7, 8, o.PWave, ABSTRACTED, _p_qrs_tconst)
EXTRASYSTOLE_PATTERN.add_transition(8, 9, o.TWave, ABSTRACTED, _t_qrs_tconst, _extrasyst_gconst)
EXTRASYSTOLE_PATTERN.add_transition(7, 9, o.TWave, ABSTRACTED, _t_qrs_tconst, _extrasyst_gconst)
...
EXTRASYSTOLE_PATTERN.add_transition(3, 10, o.QRS, ABSTRACTED, _qrs_ext_tconst(True), _qrs_ext_gconst_npause)
EXTRASYSTOLE_PATTERN.add_transition(10, 11, o.TWave, ABSTRACTED, _t_qrs_tconst)
EXTRASYSTOLE_PATTERN.add_transition(10, 12, o.QRS, ABSTRACTED, _qrs_fin_npause_tconst)
EXTRASYSTOLE_PATTERN.add_transition(11, 12, o.QRS, ABSTRACTED, _qrs_fin_npause_tconst)
EXTRASYSTOLE_PATTERN.add_transition(12, 13, o.PWave, ABSTRACTED, _p_qrs_tconst)
EXTRASYSTOLE_PATTERN.add_transition(13, 14, o.TWave, ABSTRACTED, _t_qrs_tconst, _extrasyst_gconst)
EXTRASYSTOLE_PATTERN.add_transition(12, 14, o.TWave, ABSTRACTED, _t_qrs_tconst, _extrasyst_gconst)
