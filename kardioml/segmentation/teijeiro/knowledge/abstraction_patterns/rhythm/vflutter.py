...
from kardioml.segmentation.teijeiro.utils.signal_processing import fft_filt
...
def _contains_qrs(pattern):
    ...
    if len(defls) > 1:
        ...
        for lead in leads:
            ...
            sig[lead] = sig_buf.get_signal_fragment(limit, defls[-1].earlystart, lead=lead)[0]
...
def _prev_rhythm_tconst(pattern, rhythm):
    ...
    tnet.add_constraint(pattern.hypothesis.start, pattern.hypothesis.end, Iv(C.VFLUT_MIN_DUR, np.inf))
...
def _vflut_gconst(pattern, _):
    ...
    if len(defls) > 1:
        ...
        hyp.meas = o.CycleMeasurements((np.mean(rrs), np.std(rrs)), (0, 0), (0, 0))
...
VFLUTTER_PATTERN.add_transition(0, 1, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst)
VFLUTTER_PATTERN.add_transition(1, 2, o.QRS, ENVIRONMENT, _qrs0_tconst)
VFLUTTER_PATTERN.add_transition(2, 3, o.Deflection, ABSTRACTED, _def0_tconst, _vflut_gconst)
VFLUTTER_PATTERN.add_transition(3, 3, o.Deflection, ABSTRACTED, _deflection_tconst, _vflut_gconst)
VFLUTTER_PATTERN.add_transition(3, 4, o.QRS, ABSTRACTED, _qrs_tconst, _vflut_gconst)
