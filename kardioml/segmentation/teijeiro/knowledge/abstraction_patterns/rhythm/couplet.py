...
from kardioml.segmentation.teijeiro.model import verify, Interval as Iv
...
from kardioml.segmentation.teijeiro.utils.signal_processing.xcorr_similarity import signal_unmatch
...
def _v0_tconst(pattern, qrs):
    ...
    if idx > 0:
        ...
        tnet.add_constraint(prev.time, qrs.time, Iv(C.TACHY_RR.start, max(C.TACHY_RR.start, refrr - adv)))
...
def _n1_tconst(pattern, qrs):
    ...
    mincompause = max(C.COMPAUSE_MIN_DUR, min(minrr * C.ICOUPLET_MIN_RREXT_F, minrr + C.ICOUPLET_MIN_RREXT))
...
def _couplet_gconst(pattern, _):
    ...
    beats = pattern.evidence[o.QRS]
    mpt = beats[0].time.start + (beats[-1].time.start - beats[0].time.start) / 2.0
    verify(
        abs(mpt - beats[2].time.start) >= C.ICOUPLET_RCHANGE
        or signal_unmatch(beats[2].shape, beats[-1].shape)
    )
    pattern.hypothesis.meas = copy.copy(pattern.evidence[o.Cardiac_Rhythm][0].meas)
...
def _t_tconst(pattern, twave):
    ...
    try:
        ...
        if qidx > 1:
            refrr = (qrs.time.end - pattern.evidence[o.QRS][qidx - 2].time.start) / 2.0
