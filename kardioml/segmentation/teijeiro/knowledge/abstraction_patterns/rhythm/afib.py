...
from kardioml.segmentation.teijeiro.knowledge.abstraction_patterns.segmentation.pwave import delineate_pwave
from kardioml.segmentation.teijeiro.knowledge.abstraction_patterns.rhythm.regular import _check_missed_beats
from kardioml.segmentation.teijeiro.knowledge.abstraction_patterns.segmentation import characterize_baseline
from kardioml.segmentation.teijeiro.knowledge.abstraction_patterns.rhythm.vflutter import _is_VF
from kardioml.segmentation.teijeiro.utils.units_helper import msec2samples as ms2sp, samples2sec as sp2sc
from kardioml.segmentation.teijeiro.utils.signal_processing.xcorr_similarity import signal_match
import copy
import numpy as np
...
Sij = np.array([[-0.075, -1.460, 0.346], [-0.806, 0.256, -0.304], [0.828, -1.926, 0.426]])
...
def is_afib_rhythm_lian(rrs):
    ...
    if len(rrs) < 3:
        ...
        return isafib
    drr = np.diff(rrs)
    xbins = np.arange(int(np.min(rrs) - ms2sp(50)), int(np.max(rrs) + ms2sp(50)), int(ms2sp(25)))
    ybins = np.arange(int(np.min(drr) - ms2sp(50)), int(np.max(drr) + ms2sp(50)), int(ms2sp(25)))
...
def _prev_afib_tconst(pattern, afib):
    ...
    pattern.last_tnet.add_constraint(afib.end, pattern.hypothesis.start, Iv(0, C.AFIB_MAX_DELAY))
...
def _prev_rhythm_tconst(pattern, rhythm):
    ...
    tnet.add_constraint(pattern.hypothesis.start, pattern.hypothesis.end, Iv(7 * C.TACHY_RR.start, np.inf))
...
def _qrs0_tconst(pattern, qrs):
    ...
    tnet.add_constraint(pattern.hypothesis.start, pattern.hypothesis.end, Iv(5 * C.TACHY_RR.start, np.inf))
...
def get_t_tconst(qrsidx):
    ...
    def _t_tconst(pattern, twave):
        ...
        if qidx > 0:
            ...
            try:
                tnet.add_constraint(qrs.time, twave.end, Iv(0, refrr - C.TQ_INTERVAL_MIN))
            except ValueError:
                pass
...
def _update_measures(pattern):
    ...
    if len(obseq) > 1 and isinstance(obseq[-2], o.TWave) and obseq[-2] is not pattern.finding:
        twave = obseq[-2]
        # Current RR measure (bounded)
        qrs = next((q for q in reversed(beats) if q.lateend <= twave.earlystart), None)
    ...
    pattern.hypothesis.meas = o.CycleMeasurements((np.mean(rrs), np.std(rrs)), (rtmean, rtstd), (0.0, 0.0))
...
def _get_pwave_sig(beg, end):
    ...
    for lead in pwave.amplitude:
        sig = sig_buf.get_signal_fragment(est + limits.start, est + limits.end + 1, lead=lead)[0]
...
def _verify_atrial_activity(pattern):
    ...
    for i in range(idx + 1, len(obseq)):
        if isinstance(obseq[i], o.QRS):
            beg = next(obs for obs in reversed(obseq[:i]) if obs is not None).lateend
...
def _qrs_gconst(pattern, _):
    ...
    if len(beats) == 2 and envrhythms:
        ...
        verify(not refrr - 2 * stdrr <= beats[1].time.start - beats[0].time.start <= refrr + 2 * stdrr)
    if len(beats) >= 3:
...
AFIB_PATTERN.add_transition(0, 1, o.Cardiac_Rhythm, ENVIRONMENT, _prev_afib_tconst, _prev_afib_gconst)
AFIB_PATTERN.add_transition(
    1, 1, o.Cardiac_Rhythm, ENVIRONMENT, _prev_multrhythm_tconst, _prev_rhythm_gconst
)
AFIB_PATTERN.add_transition(1, 2, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst, _prev_rhythm_gconst)
AFIB_PATTERN.add_transition(0, 2, o.Cardiac_Rhythm, ENVIRONMENT, _prev_rhythm_tconst, _prev_rhythm_gconst)
...
for i in range(C.AFIB_MIN_NQRS - 4):
    AFIB_PATTERN.add_transition(9 + i, 10 + i, o.QRS, ABSTRACTED, _qrs_tconst, _qrs_gconst)
# T waves are searched after the necessary QRS have been observed.
ST = 9 + C.AFIB_MIN_NQRS - 4
for i in range(C.AFIB_MIN_NQRS - 2):
    AFIB_PATTERN.add_transition(ST, ST + 1, o.TWave, ABSTRACTED, get_t_tconst(i + 1))
...
AFIB_PATTERN.add_transition(ST, ST + 1)
AFIB_PATTERN.add_transition(ST + 1, ST, o.QRS, ABSTRACTED, _qrs_tconst, _qrs_gconst)
